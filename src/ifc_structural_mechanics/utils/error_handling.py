...
from typing import Callable, Dict, List, Optional, Any, Tuple, Pattern, Union
...
class EntityErrorMapper:
    """
    Maps errors to domain entities using configurable entity mappings.

    This class provides a more advanced entity mapping system than the
    basic mapper capabilities in the ErrorDetector class.
    """

    def __init__(self, domain_model: Optional[Any] = None):
        """
        Initialize an EntityErrorMapper.

        Args:
            domain_model (Optional[Any]): Domain model to associate errors with.
        """
    ...
    def add_mapping(
        ...
        """
        Add a mapping between analysis entity ID and domain entity ID.

        Args:
            entity_type (EntityType): Type of entity being mapped.
            analysis_id (Union[str, int]): ID of the entity in the analysis software.
            domain_id (Union[str, int]): ID of the entity in the domain model.
        """
        type_key = str(entity_type)
        if type_key not in self.mapping_tables:
            self.mapping_tables[type_key] = {}

        self.mapping_tables[type_key][str(analysis_id)] = str(domain_id)

    def bulk_add_mappings(self, mappings: Dict[str, Dict[str, str]]) -> None:
        """
        Add multiple mappings at once.

        Args:
            mappings (Dict[str, Dict[str, str]]): Dictionary of entity types to
                dictionaries mapping analysis IDs to domain IDs.
        """
        for entity_type, id_mappings in mappings.items():
            if entity_type not in self.mapping_tables:
                self.mapping_tables[entity_type] = {}

            self.mapping_tables[entity_type].update(id_mappings)

    def get_domain_id(
        self, entity_type: EntityType, analysis_id: Union[str, int]
    ) -> Optional[str]:
        """
        Get the domain ID corresponding to an analysis entity ID.
        ...
        Returns:
            Optional[str]: Domain ID if found, None otherwise.
        """
        type_key = str(entity_type)
        id_key = str(analysis_id)

        if type_key not in self.mapping_tables:
            return None

        return self.mapping_tables[type_key].get(id_key)

    def get_analysis_id(
        self, entity_type: EntityType, domain_id: Union[str, int]
    ) -> Optional[str]:
        """
        Get the analysis entity ID corresponding to a domain ID.
        ...
        Returns:
            Optional[str]: Analysis ID if found, None otherwise.
        """
        type_key = str(entity_type)
        ...
        # Reverse lookup in the mapping table
        for analysis_id, mapped_domain_id in self.mapping_tables[type_key].items():
            if mapped_domain_id == domain_id_str:
                return analysis_id

        return None

    def map_error_to_entity(
        self, error_text: str, error_patterns: Optional[List[ErrorPattern]] = None
    ) -> Dict[str, Any]:
        """
        Map an error message to an entity in the domain model.

        Args:
            error_text (str): The error message to map.
            error_patterns (Optional[List[ErrorPattern]]): Error patterns to use for mapping.
                If None, default patterns are used.

        Returns:
            Dict[str, Any]: Mapping information including entity_type, entity_id, and domain_id.
        """
        if error_patterns is None:
            # Use default patterns from the registry
            registry = ErrorPatternRegistry()
            error_patterns = registry.get_patterns(
                software="calculix", pattern_type="error"
            )

        for pattern in error_patterns:
            match = pattern.match(error_text)
            if match and pattern.entity_type and match.groups():
                try:
                    entity_id = match.group(1)
                    # Convert to integer for node and element types
                    if pattern.entity_type in [EntityType.NODE, EntityType.ELEMENT]:
                    ...
                    domain_id = self.get_domain_id(pattern.entity_type, entity_id)

                    return {
                        "entity_type": str(pattern.entity_type),
                        "entity_id": entity_id,
                        "domain_id": domain_id,
                    }
                except (IndexError, ValueError):
        ...
        return {"entity_type": None, "entity_id": None, "domain_id": None}
...
def format_error_message(
    ...
    """
    if isinstance(error, StructuralAnalysisError):
        # StructuralAnalysisError already has a nice string representation
        if context:
...
class ErrorCollector:
    ...
    This class provides a way to collect errors and warnings during
    processing without immediately raising exceptions.
    """

    def __init__(self, raise_on_critical: bool = True):
        """
        Initialize an ErrorCollector.

        Args:
            raise_on_critical (bool): Whether to raise an exception immediately
                when a critical error is added.
        """
        self.errors = []
        self.warnings = []
        self.info = []
    ...
    def add_error(
        ...
        **kwargs,
        ...
        Raises:
            StructuralAnalysisError: If raise_on_critical is True and severity is CRITICAL.
        """
        error_info = {
            "message": message,
            "severity": severity,
        ...
        if severity == ErrorSeverity.WARNING:
            ...
            # Raise an exception for critical errors if configured to do so
    ...
    def add_warning(
        ...
        **kwargs,
        ...
        Args:
            ...
            entity_type (Optional[EntityType]): Type of entity related to the warning.
            entity_id (Optional[str]): ID of the entity in the analysis software.
            domain_id (Optional[str]): ID of the entity in the domain model.
            **kwargs: Additional context information.
        """
    ...
    def add_info(
        ...
        **kwargs,
        ...
        Args:
            ...
            entity_type (Optional[EntityType]): Type of entity related to the info.
            entity_id (Optional[str]): ID of the entity in the analysis software.
            domain_id (Optional[str]): ID of the entity in the domain model.
            **kwargs: Additional context information.
        """
    ...
    def has_errors(self, include_warnings: bool = False) -> bool:
        ...
        Args:
            include_warnings (bool): Whether to include warnings as errors.
    ...
    def get_all(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        Returns:
            Dict[str, List[Dict[str, Any]]]: Dictionary with 'errors', 'warnings', and 'info' lists.
        """
    ...
    def summary(self) -> str:
        ...
        Returns:
            str: Summary of errors and warnings.
        """
        summary = []

        if self.errors:
            summary.append(f"Found {len(self.errors)} errors:")
        ...
        if self.info:
            ...
            for i, info in enumerate(self.info, 1):
                summary.append(f"  {i}. {info['message']}")

        if not self.errors and not self.warnings and not self.info:
            summary.append("No errors, warnings, or info messages collected.")

        return "\n".join(summary)


def traceable_errors(func: Callable) -> Callable:
    """
    Decorator to add traceability to errors.

    This decorator wraps a function to catch exceptions,
    add trace information, and re-raise them as StructuralAnalysisError.
    ...
    def wrapper(*args, **kwargs):
        ...
        except StructuralAnalysisError:
            # Already a structural analysis error, just re-raise
            raise
        except Exception as e:
            # Get traceback information
            tb = traceback.format_exc()

            # Wrap in StructuralAnalysisError with traceback
            raise StructuralAnalysisError(
